...
namespace librbd{
  namespace cache{
     ...
     CacheManager* CacheManager::Instance(CephContext* m_cct){
    	...
    	void CacheManager::insert(CephContext* m_cct, uint64_t image_extents_addr, ceph::bufferlist bl){
      ...
      ceph::bufferlist CacheManager::get(CephContext* m_cct, uint64_t image_extent_addr){
    		ldout(m_cct, 20) << "reading from cache for the image extent :: " << image_extent_addr << dendl;
    		ceph::bufferlist bl = realCache->get(m_cct, image_extent_addr);
    		return bl;
    	}
    }
}
